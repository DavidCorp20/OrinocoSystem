export const fmtMoney = (n, currency = "USD") => {
  try {
    return new Intl.NumberFormat("es", { style: "currency", currency, maximumFractionDigits: 2 }).format(Number(n || 0));
  } catch {
    return `$${Number(n || 0).toFixed(2)}`;
  }
};

export const fmtNum = (n) => Number(n || 0).toLocaleString("es", { maximumFractionDigits: 2 });
...
export const BUSINESS_TYPES = [
  { value: "abarrotes", label: "Abarrotes / Bodega / Minimarket" },
  ...
  { value: "alimentos", label: "Tienda de alimentos" },
  { value: "otro", label: "Otro emprendimiento" },
];
...
export const stockStatus = (p) => {
  if (p.stock <= 0) return { key: "agotado", label: "Agotado", cls: "bg-rose-100 text-rose-800 border-rose-300" };
  if (p.stock <= (p.min_stock ?? 0)) return { key: "bajo", label: "Stock bajo", cls: "bg-amber-100 text-amber-800 border-amber-300" };
  return { key: "ok", label: "OK", cls: "bg-emerald-100 text-emerald-800 border-emerald-300" };
};
