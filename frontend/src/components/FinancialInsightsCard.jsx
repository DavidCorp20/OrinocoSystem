...
function tone(level) {
  if (level === "urgent" || level === "warning" || level === "rojo") return "border-rose-200 bg-rose-50 text-rose-900";
  if (level === "attention" || level === "yellow" || level === "amarillo") return "border-amber-200 bg-amber-50 text-amber-900";
...
export default function FinancialInsightsCard({ currency = "USD" }) {
  ...
  if (!data) {
    return <div className="h-40 bg-card border border-border rounded-2xl animate-pulse" data-testid="financial-insights-loading" />;
  }
    ...
    <section className="bg-card border border-border rounded-2xl p-5 md:p-6 space-y-5" data-testid="financial-insights-card">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {data.headline && <p className="mt-2 text-sm text-slate-600 max-w-3xl">{data.headline}</p>}
        </div>
        <span className="text-xs font-semibold text-muted-foreground whitespace-nowrap">Últimos 30 días</span>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {facts.revenue != null && <Metric label="Ventas" value={fmtMoney(facts.revenue, currency)} />}
        {facts.gross_profit != null && <Metric label="Utilidad bruta" value={fmtMoney(facts.gross_profit, currency)} />}
        {facts.gross_margin != null && <Metric label="Margen bruto" value={fmtPct(facts.gross_margin)} />}
        {facts.operating_profit != null && <Metric label="Utilidad operativa" value={fmtMoney(facts.operating_profit, currency)} />}
      </div>
        ...
        <div className="space-y-2">
          {insights.slice(0, 4).map((item, i) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100" data-testid={`financial-insight-${i}`}>
              {item.type === "positive" ? <TrendingUp className="w-4 h-4 mt-0.5 text-emerald-600 shrink-0" /> : <TrendingDown className="w-4 h-4 mt-0.5 text-slate-500 shrink-0" />}
              <p className="text-sm text-slate-700">{typeof item === "string" ? item : (item.text || item.message || item.insight)}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {warnings.length > 0 && (
        <div className="space-y-2" data-testid="financial-warnings">
          ...
          {warnings.slice(0, 3).map((item, i) => (
            <div key={i} className={`border rounded-xl p-3 text-sm ${tone(item.level || item.severity)}`}>
              {typeof item === "string" ? item : (item.text || item.message || item.warning)}
            </div>
          ))}
        </div>
      )}
      ...
      {actions.length > 0 && (
        <div className="space-y-2" data-testid="financial-actions">
          ...
          {actions.slice(0, 3).map((item, i) => (
            <div key={i} className="flex items-start gap-3 text-sm text-slate-700">
              ...
              <span>{typeof item === "string" ? item : (item.text || item.message || item.action)}</span>
            </div>
          ))}
        </div>
      )}
