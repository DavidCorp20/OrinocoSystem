...
const menuFor=(text,isFirst=false)=>{
 ...
 if(isFirst||/puedo revisar tus ventas y productos|que quieres revisar|por donde empezamos/.test(t)) return ["Ventas","Productos","Inventario","Ganancias","Proyección"];
 ...
 if(/reposicion|reponer|poco stock|comprar/.test(t)) return ["Ver faltantes","Ver poco stock","Ver productos"];
 if(/proyeccion|proximos/.test(t)) return ["Ver tendencia","Ver productos","Ver ventas"];
 if(/ganancia|margen/.test(t)) return ["Comparar ganancias","Productos más vendidos","Inventario"];
 return [];
};
const mapChoice=choice=>choice;
export default function AIAssistant({open,onClose}){
 ...
 const send=async text=>{let msg=(text??input).trim();if(!msg||sending)return;
   ...
   // Make a typed "sí" follow the question Cubi just asked instead of repeating the previous intent.
