...
import { AlertTriangle, CheckCircle2, Calculator, Clock3, History, Save, WalletCards } from "lucide-react";
  ...
  const load = async target => {
    ...
    try {
      const [{ data: s }, { data: h }] = await Promise.all([
        api.get(`/cash-closures/summary?date=${target}`),
        api.get("/cash-closures?limit=30"),
      ]);
      setSummary(s);
      setClosures(h.closures || []);
      ...
      toast.error(apiError(e, "No pude cargar el cierre de caja."));
  ...
  const save = async () => {
    ...
    try {
      ...
      await api.post("/cash-closures", {
        date,
        opening_cash: effectiveOpening,
        denominations,
        counted_cash: counted,
        other_cash_in: Number(otherIn || 0),
        other_cash_out: Number(otherOut || 0),
        observations: observations || null,
        use_previous_closing: usePrevious,
      });
  ...
  return <div className="space-y-5 max-w-6xl" data-testid="cierre-caja-page">
    ...
    <div className="bg-card border rounded-2xl p-5">
      <div className="flex flex-wrap items-end gap-4">
        <div><Label>Fecha de cierre</Label><Input type="date" value={date} onChange={e => setDate(e.target.value)} className="mt-1.5 w-44" /></div>
        <div className="flex items-center gap-2 h-10"><input id="previous" type="checkbox" checked={usePrevious} onChange={e => setUsePrevious(e.target.checked)} /><Label htmlFor="previous">Usar cierre anterior como apertura</Label></div>
        {!usePrevious && <div><Label>Efectivo inicial</Label><Input type="number" min="0" step="0.01" value={opening} onChange={e => setOpening(e.target.value)} className="mt-1.5 w-44" /></div>}
      </div>
    </div>

    <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4">
      <Metric title="Ventas en efectivo" value={money(summary?.cash_sales)} detail={`${summary?.sales_count || 0} ventas registradas`} />
      <Metric title="Cobros en efectivo" value={money(summary?.cash_receivables)} detail="Cuentas por cobrar" />
      <Metric title="Compras en efectivo" value={money(summary?.cash_purchases)} detail={`${summary?.purchases_count || 0} compras registradas`} negative />
      <Metric title="Gastos en efectivo" value={money(summary?.cash_expenses)} detail={`${summary?.expenses_count || 0} gastos registrados`} negative />
    </div>

    <div className="grid lg:grid-cols-2 gap-5">
      <div className="bg-card border rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4"><Calculator className="w-5 h-5 text-primary"/><h2 className="font-heading font-bold">Conciliación</h2></div>
        <div className="space-y-3 text-sm">
          <Row label="Efectivo inicial" value={money(effectiveOpening)} />
          <Row label="+ Ventas en efectivo" value={money(summary?.cash_sales)} />
          <Row label="+ Cobros en efectivo" value={money(summary?.cash_receivables)} />
          <Row label="− Compras en efectivo" value={money(summary?.cash_purchases)} />
          <Row label="− Pagos de cuentas por pagar" value={money(summary?.cash_payables)} />
          <Row label="− Gastos en efectivo" value={money(summary?.cash_expenses)} />
          <div className="border-t pt-3"><Row label="Efectivo esperado" value={money(expected)} strong /></div>
          <div className="grid grid-cols-2 gap-3 pt-2"><div><Label>+ Otros ingresos</Label><Input type="number" min="0" step="0.01" value={otherIn} onChange={e => setOtherIn(e.target.value)} className="mt-1.5" /></div><div><Label>− Otros retiros</Label><Input type="number" min="0" step="0.01" value={otherOut} onChange={e => setOtherOut(e.target.value)} className="mt-1.5" /></div></div>
        </div>
      </div>

      <div className="bg-card border rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4"><WalletCards className="w-5 h-5 text-primary"/><h2 className="font-heading font-bold">Conteo físico</h2></div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {DENOMINATIONS.map(value => <div key={value}><Label>{value < 1 ? `${value.toFixed(2)} Bs` : `${value} Bs`}</Label><Input type="number" min="0" step="1" value={denoms[String(value)]} onChange={e => setDenoms({...denoms, [String(value)]: e.target.value})} className="mt-1.5" /></div>)}
        </div>
        <div className="mt-4 rounded-xl bg-secondary/60 p-4 flex justify-between"><span className="font-semibold">Efectivo contado</span><span className="font-heading font-extrabold">{money(counted)}</span></div>
        <div className={`mt-3 rounded-xl p-4 flex items-center justify-between border ${difference === 0 ? "bg-emerald-50 border-emerald-200" : difference > 0 ? "bg-blue-50 border-blue-200" : "bg-rose-50 border-rose-200"}`}>
          <div className="flex items-center gap-2 font-semibold">{difference === 0 ? <CheckCircle2 className="w-5 h-5"/> : <AlertTriangle className="w-5 h-5"/>}<span>{difference === 0 ? "Caja cuadrada" : difference > 0 ? "Sobrante" : "Faltante"}</span></div><span className="font-heading font-extrabold">{money(Math.abs(difference))}</span>
        </div>
      </div>
    </div>
