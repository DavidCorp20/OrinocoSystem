...
SUB_STATUSES = {"activo", "pendiente", "vencido", "cancelado"}
...
async def platform_overview(user: dict = Depends(require_superadmin)):
    ...
    now = datetime.now(timezone.utc); d30 = (now - timedelta(days=30)).isoformat(); month = now.strftime("%Y-%m")
...
async def pending_users(user: dict = Depends(require_superadmin)):
    return {"users": await db.users.find({"platform_role":{"$ne":"superadmin"},"approved":{"$ne":True}},{"_id":0,"password_hash":0}).sort("created_at",-1).to_list(5000)}
...
async def list_plans(user:dict=Depends(require_superadmin)):
    return {"plans":await db.platform_plans.find({}, {"_id":0}).sort("monthly_price_usd",1).to_list(100)}
...
async def assign_subscription(business_id:str,data:PlatformSubscriptionIn,user:dict=Depends(require_superadmin)):
    ...
    price=round(data.monthly_price_usd or plan["monthly_price_usd"],2); now=now_iso()
    sub={"id":new_id(),"business_id":business_id,"plan_id":plan["id"],"plan_name":plan["name"],"status":data.status,"monthly_price_usd":price,"due_date":data.due_date,"created_at":now,"updated_at":now}
    await db.platform_subscriptions.update_many({"business_id":business_id,"status":{"$ne":"cancelado"}},{"$set":{"status":"cancelado","updated_at":now}}); await db.platform_subscriptions.insert_one(sub)
    await db.businesses.update_one({"id":business_id},{"$set":{"plan_id":plan["id"],"plan_name":plan["name"],"subscription_status":data.status,"subscription_due_date":data.due_date,"monthly_price_usd":price}}); sub.pop("_id",None); return {"subscription":sub}
...
async def subscription_status(subscription_id:str,status:str,user:dict=Depends(require_superadmin)):
    ...
    await db.platform_subscriptions.update_one({"id":subscription_id},{"$set":{"status":status,"updated_at":now_iso()}}); await db.businesses.update_one({"id":sub["business_id"]},{"$set":{"subscription_status":status}})
    return {"subscription":{**sub,"status":status}}
...
async def create_billing(data:PlatformBillingIn,user:dict=Depends(require_superadmin)):
    ...
    if not sub: raise HTTPException(404,"Suscripción no encontrada")
    paid_at=data.paid_at or now_iso(); bill={"id":new_id(),"subscription_id":sub["id"],"business_id":sub["business_id"],"amount":round(data.amount,2),"payment_method":data.payment_method,"paid_at":paid_at,"notes":data.notes,"created_at":now_iso(),"created_by":user["id"]}
    await db.platform_billing.insert_one(bill)
    ...
    if due:
        ...
        except ValueError: next_due=None
...
async def billing_metrics(user:dict=Depends(require_superadmin)):
    now=datetime.now(timezone.utc); month=now.strftime("%Y-%m"); subs=await db.platform_subscriptions.find({}, {"_id":0}).to_list(10000); paid=await db.platform_billing.find({}, {"_id":0}).to_list(10000); expenses=await db.platform_expenses.find({}, {"_id":0}).to_list(5000)
    active=[s for s in subs if s.get("status")=="activo"]; mrr=round(sum(float(s.get("monthly_price_usd",0) or 0) for s in active),2); revenue=round(sum(float(x.get("amount",0) or 0) for x in paid if _month(x.get("paid_at"))==month),2); costs=round(sum(float(e.get("amount",0) or 0) for e in expenses if _month(e.get("date") or e.get("created_at"))==month),2)
    due=sorted([s for s in subs if s.get("status") in {"pendiente","vencido"}],key=lambda x:x.get("due_date") or "")[:100]
    return {"month":month,"active_customers":len(active),"mrr_usd":mrr,"projected_revenue_usd":mrr,"collected_revenue_usd":revenue,"platform_costs_usd":costs,"projected_net_usd":round(mrr-costs,2),"cash_net_usd":round(revenue-costs,2),"due_soon":due}
