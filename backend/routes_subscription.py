...
async def my_subscription(user: dict = Depends(require_roles("propietario", "administrador"))):
    ...
    entitlements = await get_entitlements(user["business_id"])
    return {
...
async def submit_payment(data: MerchantPaymentIn, user: dict = Depends(require_roles("propietario", "administrador"))):
    ...
    if data.amount_usd <= 0: raise HTTPException(400, "El monto USD debe ser mayor que cero")
    if data.bcv_rate <= 0 or data.market_rate <= 0: raise HTTPException(400, "Las tasas deben ser mayores que cero")
    average_rate = round((data.bcv_rate + data.market_rate) / 2, 4);amount_bs = round(data.amount_usd * average_rate, 2);paid_at = data.paid_at or now_iso()
    payment={"id":new_id(),"subscription_id":sub["id"],"business_id":user["business_id"],"amount":round(data.amount_usd,2),"amount_usd":round(data.amount_usd,2),"amount_bs":amount_bs,"payment_method":data.payment_method,"reference":data.reference,"paid_at":paid_at,"bcv_rate":data.bcv_rate,"market_rate":data.market_rate,"average_rate":average_rate,"rate_split":0.5,"status":"pendiente_verificacion","notes":data.notes,"created_at":now_iso(),"created_by":user["id"]}
