...
from plan_access import DEFAULT_ENTITLEMENTS, get_entitlements
...
PLAN_PATHS={
 "/finance":"finance", "/obligations":"obligations", "/reports":"reports_advanced", "/projections":"projections", "/promotions":"promotions", "/recipes":"recipes", "/cash-closure":"cash_closure",
}
class PlanAccessMiddleware(BaseHTTPMiddleware):
    async def dispatch(self,request:Request,call_next):
        ...
        feature=None
        for suffix,entitlement in PLAN_PATHS.items():
            if path.startswith("/api"+suffix):feature=entitlement;break
        if feature:
            from security import get_current_user
            try:
                user=await get_current_user(request)
                if user.get("platform_role")!="superadmin" and user.get("business_id"):
                    await __import__("plan_access").require_feature(user["business_id"],feature)
...
async def startup():
    ...
    defaults=[
      {"name":"Básico","description":"Para negocios que comienzan a digitalizar su operación.","monthly_price_usd":4.98,"active":True,"features":["Inventario","Ventas","Compras","Gastos","Dashboard","Cierre de caja","Cubi básico"],"entitlements":DEFAULT_ENTITLEMENTS["Básico"]},
      {"name":"Negocio","description":"Para negocios que necesitan más control y análisis de su operación.","monthly_price_usd":8.98,"active":True,"features":["Todo Básico","Finanzas","Cuentas","Reportes avanzados","Proyección","Promociones","ABC/XYZ","Equipo hasta 3"],"entitlements":DEFAULT_ENTITLEMENTS["Negocio"]},
      {"name":"Pro","description":"Para negocios que quieren aprovechar todas las capacidades de CuadraApp.","monthly_price_usd":14.98,"active":True,"features":["Todo Negocio","Recetas","Analítica avanzada","Automatizaciones avanzadas","Cubi avanzado","Equipo hasta 10"],"entitlements":DEFAULT_ENTITLEMENTS["Pro"]}
    ]
