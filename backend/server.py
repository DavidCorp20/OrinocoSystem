...
PLAN_PATHS={"/finance":"finance","/finanzas":"finance","/obligations":"obligations","/reports":"reports_advanced","/projections":"projections","/projection":"projections","/promotions":"promotions","/recipes":"recipes","/cash-closure":"cash_closure","/cash-closures":"cash_closure"}
...
async def startup():
    ...
    defaults=[{"name":"Básico","description":"Para negocios que comienzan a digitalizar su operación.","monthly_price_usd":4.98,"active":True,"features":["Inventario","Ventas","Compras","Gastos","Dashboard","Cierre de caja","Cubi básico"],"entitlements":DEFAULT_ENTITLEMENTS["Básico"]},{"name":"Negocio","description":"Para negocios que necesitan más control y análisis de su operación.","monthly_price_usd":8.98,"active":True,"features":["Todo Básico","Finanzas","Cuentas","Reportes avanzados","Proyección","Promociones","ABC/XYZ","Equipo hasta 3"],"entitlements":DEFAULT_ENTITLEMENTS["Negocio"]},{"name":"Pro","description":"Para negocios que quieren aprovechar todas las capacidades de CuadraApp.","monthly_price_usd":14.98,"active":True,"features":["Todo Negocio","Recetas","Analítica avanzada","Automatizaciones avanzadas","Cubi avanzado","Equipo hasta 10"],"entitlements":DEFAULT_ENTITLEMENTS["Pro"]}]
    for p in defaults:
        base={k:v for k,v in p.items() if k!="entitlements"};await db.platform_plans.update_one({"name":p["name"]},{"$set":{**base,"updated_at":datetime.now(timezone.utc).isoformat()},"$setOnInsert":{"id":__import__("uuid").uuid4().hex,"created_at":datetime.now(timezone.utc).isoformat(),"entitlements":p["entitlements"]}},upsert=True)
        await db.platform_plans.update_one({"name":p["name"],"entitlements":{"$exists":False}},{"$set":{"entitlements":p["entitlements"]}})
