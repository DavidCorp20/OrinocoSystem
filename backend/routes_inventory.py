...
async def create_movement(data: MovementIn, user: dict = Depends(require_business)):
    ...
    if data.type not in ("entrada", "salida"):
    ...
    if data.reason not in valid:
        raise HTTPException(status_code=400, detail="Motivo inválido para este tipo de movimiento")
    ...
    if data.type == "salida":
        updated = await db.products.find_one_and_update(
            {"id": data.product_id, "business_id": bid, "stock": {"$gte": data.quantity}},
            {"$inc": {"stock": -data.quantity}, "$set": {"updated_at": now_iso()}},
            return_document=ReturnDocument.AFTER,
        )
        if not updated:
            raise HTTPException(status_code=400, detail=f"Stock insuficiente: solo hay {product['stock']:g} unidad(es) de {product['name']}")
    else:
        updated = await db.products.find_one_and_update(
    ...
    return {"movement": movement, "stock": new_stock}
