...
async def record_cash_movement(*, business_id: str, direction: str, movement_type: str, source_type: str, source_id: str, amount: float, payment_method: str, user_email: Optional[str] = None, occurred_at: Optional[str] = None, currency: Optional[str] = None, notes: Optional[str] = None) -> dict[str, Any]:
    ...
    idempotency_key=f"{business_id}:{source_type}:{source_id}:{method}:{direction}"
    ...
    doc={"id":new_id(),"business_id":business_id,"direction":direction,"type":movement_type,"source_type":source_type,"source_id":source_id,"amount":amount,"payment_method":method,"currency":currency,"idempotency_key":idempotency_key,"occurred_at":occurred_at or now_iso(),"user_email":user_email,"notes":notes,"created_at":now_iso()}
    result=await db.cash_movements.update_one({"idempotency_key":idempotency_key},{"$setOnInsert":doc},upsert=True)
    if result.upserted_id is not None:return doc
    existing=await db.cash_movements.find_one({"idempotency_key":idempotency_key},{"_id":0})
