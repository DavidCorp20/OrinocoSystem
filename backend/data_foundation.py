...
async def ensure_data_foundation():
    ...
    for collection, fields in indexes.items():
        try: await db[collection].create_index(fields)
        except Exception: pass
    try:
        await db.cash_movements.create_index([("idempotency_key", ASCENDING)], unique=True, partialFilterExpression={"idempotency_key":{"$exists":True}})
    except Exception: pass
    ...
    async for product in db.products.find({"business_id":{"$exists":True},"category":{"$nin":[None,""]}},{"business_id":1,"category":1}):
        ...
        if name: await db.categories.update_one({"business_id":product["business_id"],"name":name},{"$setOnInsert":{"id":new_id(),"business_id":product["business_id"],"name":name,"active":True,"created_at":_now()}},upsert=True)

    async for purchase in db.purchases.find({"business_id":{"$exists":True},"supplier":{"$nin":[None,""]}},{"business_id":1,"supplier":1,"supplier_rif":1}): await _ensure_supplier_doc(purchase["business_id"],purchase.get("supplier"),purchase.get("supplier_rif"))
    async for product in db.products.find({"business_id":{"$exists":True},"supplier":{"$nin":[None,""]}},{"business_id":1,"supplier":1}): await _ensure_supplier_doc(product["business_id"],product.get("supplier"),None)
    ...
    async for sale in db.sales.find({"business_id":{"$exists":True},"customer_name":{"$nin":[None,""]}},{"business_id":1,"customer_name":1,"customer_rif":1}):
        ...
        if name: await db.customers.update_one({"business_id":sale["business_id"],"$or":[{"rif":sale.get("customer_rif")} ,{"name":name}]},{"$setOnInsert":{"id":new_id(),"business_id":sale["business_id"],"name":name,"rif":sale.get("customer_rif"),"active":True,"created_at":_now(),"updated_at":_now()}},upsert=True)
    ...
    async for purchase in db.purchases.find({"business_id":{"$exists":True},"supplier":{"$nin":[None,""]}},{"id":1,"business_id":1,"supplier":1,"supplier_rif":1,"created_at":1,"total":1,"items":1,"status":1}):
        ...
        if supplier: await db.supplier_events.update_one({"business_id":purchase["business_id"],"source_id":purchase.get("id"),"event_type":"purchase"},{"$setOnInsert":{"id":new_id(),"business_id":purchase["business_id"],"supplier_id":supplier["id"],"event_type":"purchase","source_id":purchase.get("id"),"event_date":purchase.get("created_at") or _now(),"amount":float(purchase.get("total",0) or 0),"status":purchase.get("status","desconocido"),"items_count":len(purchase.get("items",[])),"created_at":_now()}},upsert=True)


async def _ensure_supplier_doc(business_id,name,rif=None):
    name=str(name or "").strip()
    if not name:return None
    query={"business_id":business_id,"name":name}; doc=await db.suppliers.find_one(query,{"_id":0})
    if doc:
        if rif and not doc.get("rif"): await db.suppliers.update_one(query,{"$set":{"rif":rif,"updated_at":_now()}}); doc["rif"]=rif
        return doc
    doc={"id":new_id(),"business_id":business_id,"name":name,"rif":rif,"active":True,"created_at":_now(),"updated_at":_now()}; await db.suppliers.insert_one(doc.copy()); return doc
...
async def ensure_customer(business_id,name,rif=None,phone=None,email=None):
    ...
    if doc:return doc
    doc={"id":new_id(),"business_id":business_id,"name":name,"rif":rif,"phone":phone,"email":email,"active":True,"created_at":_now(),"updated_at":_now()}; await db.customers.insert_one(doc.copy()); return doc
...
async def ensure_supplier(business_id,name,rif=None,phone=None,email=None):
    ...
    if doc and (phone or email): await db.suppliers.update_one({"id":doc["id"],"business_id":business_id},{"$set":{"phone":phone or doc.get("phone"),"email":email or doc.get("email"),"updated_at":_now()}})
    return doc
