...
async def create_sale(data:SaleIn,user:dict=Depends(require_business)):
    ...
    for item in data.items:
        ...
        unit=(item.unit or product.get("unit") or "unidad").lower();base_unit=(product.get("base_unit") or product.get("unit") or "unidad").lower();conversion=factor(unit)/factor(base_unit)
        base_qty=round(item.quantity*conversion,6)
    ...
    business=await db.businesses.find_one({"id":bid},{"_id":0}) or {};subtotal=round(subtotal,2);cost_total=round(cost_total,2);iva_enabled=business.get("iva_enabled",False);iva_percent=float(business.get("iva_percent",16) or 0) if iva_enabled else 0;iva_amount=round(subtotal-subtotal/(1+iva_percent/100),2) if iva_enabled and iva_percent else 0;delivery_enabled=bool(business.get("delivery_enabled",False));delivery_amount=round(float(business.get("delivery_amount",0) or 0),2) if delivery_enabled else 0;base_delivery=round(subtotal+delivery_amount,2);igtf_enabled=bool(business.get("igtf_enabled",False));igtf_percent=float(business.get("igtf_percent",3) or 0) if igtf_enabled else 0;foreign={"usd","divisas","zelle","tarjeta_divisa"};foreign_base=sum(p.amount for p in data.payment_parts if p.method.strip().lower() in foreign) if data.payment_parts else (base_delivery if data.payment_method.strip().lower() in foreign else 0);igtf_amount=round(float(foreign_base)*igtf_percent/100,2) if igtf_enabled else 0;total=round(base_delivery+igtf_amount,2);payments=_validate_payments(total,data.payment_method,data.payment_parts);rate=(await get_effective_rate(business)).get("rate")
