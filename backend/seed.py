...
async def _ensure_user(email: str, name: str, password: str, role="propietario", platform_role=None):
    ...
    if not existing:
        user = {
            ...
            "role": role, "platform_role": platform_role, "business_id": None,
            "approved": True if platform_role == "superadmin" else True,
            "approved_at": now_iso(), "approved_by": "system_seed", "created_at": now_iso(),
        }
        await db.users.insert_one(user)
        return user
    updates = {"approved": True, "approved_at": existing.get("approved_at") or now_iso(), "approved_by": existing.get("approved_by") or "system_seed"}
...
async def _seed_profile(user, profile, rng_seed):
    ...
    for days_ago in range(30, -1, -1):
        ...
        for _ in range(n_sales):
            ...
            for p in chosen:
                ...
                items.append({"product_id": p["id"], "name": p["name"], "quantity": qty, "unit_price": price, "discount": discount, "cost": p["purchase_price"], "line_total": line_total})
                total += line_total
                cost_total += p["purchase_price"] * qty
                stock_map[p["id"]] -= qty
                movements.append({"id": new_id(), "business_id": bid, "product_id": p["id"], "product_name": p["name"], "type": "salida", "reason": "venta", "quantity": qty, "stock_after": stock_map[p["id"]], "user_email": user["email"], "notes": "Venta (demo)", "created_at": created})
            if not items:
                continue
            total = round(total, 2)
            cost_total = round(cost_total, 2)
            sales_docs.append({"id": new_id(), "business_id": bid, "items": items, "total": total, "cost_total": cost_total, "profit": round(total - cost_total, 2), "payment_method": rng.choice(PAYMENTS), "customer_name": None, "customer_rif": None, "user_email": user["email"], "created_at": created})
    if sales_docs:
    ...
    for days_ago, picks in [(24, [6, 8, 5]), (15, [0, 4, 10]), (6, [6, 9, 2])]:
        ...
        for idx in picks:
            ...
            items.append({"product_id": p["id"], "name": p["name"], "quantity": qty, "unit_cost": p["purchase_price"], "line_total": line_total})
            total += line_total
            stock_map[p["id"]] += qty
            movements.append({"id": new_id(), "business_id": bid, "product_id": p["id"], "product_name": p["name"], "type": "entrada", "reason": "compra", "quantity": qty, "stock_after": stock_map[p["id"]], "user_email": user["email"], "notes": "Compra (demo)", "created_at": created})
        purchase_docs.append({"id": new_id(), "business_id": bid, "supplier": products[0]["supplier"], "supplier_rif": None, "items": items, "total": round(total, 2), "payment_method": "transferencia", "status": "completada", "user_email": user["email"], "created_at": created})
    if purchase_docs:
        await db.purchases.insert_many(purchase_docs)

    expense_docs = [{"id": new_id(), "business_id": bid, "category": cat, "description": desc, "amount": amount, "date": (now - timedelta(days=days_ago)).date().isoformat(), "user_email": user["email"], "created_at": ts(days_ago, 18)} for cat, desc, amount, days_ago in profile["expenses"]]
...
async def _ensure_demo_business(user, profile, rng_seed, *, force_rebuild=False):
    ...
    if not existing_business:
        bid = new_id()
        existing_business = {"id": bid, "owner_id": user["id"], "name": profile["name"], "type": profile["type"], "currency": "USD", "active": True, "bcv_mode": "auto", "bcv_rate": None, "rif": None, "address": "Venezuela", "phone": None, "created_at": now_iso()}
        await db.businesses.insert_one(existing_business)
    else:
        bid = existing_business["id"]
        await db.businesses.update_one({"id": bid}, {"$set": {"owner_id": user["id"], "name": profile["name"], "type": profile["type"], "currency": "USD", "active": True, "bcv_mode": existing_business.get("bcv_mode", "auto"), "bcv_rate": existing_business.get("bcv_rate"), "address": existing_business.get("address", "Venezuela"), "phone": existing_business.get("phone")}})
    ...
    if await db.products.count_documents({"business_id": bid}) == 0 or force_rebuild:
        ...
        for i, (name, cat, sup, cost, price, min_stock, initial, unit) in enumerate(profile["products"]):
            pid = new_id()
            products.append({"id": pid, "business_id": bid, "name": name, "sku": f"P-{i + 1:04d}", "barcode": f"775{rng.randint(1000000000, 9999999999)}", "category": cat, "brand": None, "supplier": sup, "purchase_price": cost, "sale_price": price, "stock": initial, "min_stock": min_stock, "max_stock": None, "unit": unit, "status": "activo", "created_at": (now - timedelta(days=31)).isoformat(), "updated_at": now_iso()})
        if products:
            await db.products.insert_many(products)
        stock_map = {p["id"]: p["stock"] for p in products}
        movements = []
        for p in products:
            movements.append({"id": new_id(), "business_id": bid, "product_id": p["id"], "product_name": p["name"], "type": "entrada", "reason": "carga_inicial", "quantity": p["stock"], "stock_after": p["stock"], "user_email": user["email"], "notes": None, "created_at": (now - timedelta(days=31)).isoformat()})
        def ts(days_ago, hour):
            if days_ago <= 0:
                return (now - timedelta(minutes=rng.randint(2, 480))).isoformat()
            return datetime.combine((now - timedelta(days=days_ago)).date(), time(hour, rng.randint(0, 59)), tzinfo=timezone.utc).isoformat()
        ...
        for days_ago in range(30, -1, -1):
            n_sales = rng.randint(3, 7) if (now - timedelta(days=days_ago)).weekday() >= 5 else rng.randint(2, 5)
            for _ in range(n_sales):
                chosen = rng.sample(products, k=rng.randint(1, 3)); items = []; total = cost_total = 0.0; created = ts(days_ago, rng.randint(8, 19))
                for p in chosen:
                    available = stock_map[p["id"]]
                    if available <= 0: continue
                    qty = min(rng.randint(1, 4), available)
                    if rng.random() < 0.12: qty = min(qty * rng.randint(3, 6), available)
                    if qty <= 0: continue
                    price = p["sale_price"]; discount = round(price * qty * 0.05, 2) if rng.random() < 0.15 else 0.0; line_total = round(price * qty - discount, 2)
                    items.append({"product_id": p["id"], "name": p["name"], "quantity": qty, "unit_price": price, "discount": discount, "cost": p["purchase_price"], "line_total": line_total}); total += line_total; cost_total += p["purchase_price"] * qty; stock_map[p["id"]] -= qty
                    movements.append({"id": new_id(), "business_id": bid, "product_id": p["id"], "product_name": p["name"], "type": "salida", "reason": "venta", "quantity": qty, "stock_after": stock_map[p["id"]], "user_email": user["email"], "notes": "Venta (demo)", "created_at": created})
                if items:
                    sales_docs.append({"id": new_id(), "business_id": bid, "items": items, "total": round(total, 2), "cost_total": round(cost_total, 2), "profit": round(total - cost_total, 2), "payment_method": rng.choice(PAYMENTS), "customer_name": None, "customer_rif": None, "user_email": user["email"], "created_at": created})
        if sales_docs: await db.sales.insert_many(sales_docs)
        purchase_docs = []
        for days_ago, picks in [(24, [6, 8, 5]), (15, [0, 4, 10]), (6, [6, 9, 2])]:
            items = []; total = 0.0; created = ts(days_ago, 9)
            for idx in picks:
                p = products[idx]; qty = rng.randint(20, 45); line_total = round(p["purchase_price"] * qty, 2); items.append({"product_id": p["id"], "name": p["name"], "quantity": qty, "unit_cost": p["purchase_price"], "line_total": line_total}); total += line_total; stock_map[p["id"]] += qty
                movements.append({"id": new_id(), "business_id": bid, "product_id": p["id"], "product_name": p["name"], "type": "entrada", "reason": "compra", "quantity": qty, "stock_after": stock_map[p["id"]], "user_email": user["email"], "notes": "Compra (demo)", "created_at": created})
            purchase_docs.append({"id": new_id(), "business_id": bid, "supplier": products[0]["supplier"], "supplier_rif": None, "items": items, "total": round(total, 2), "payment_method": "transferencia", "status": "completada", "user_email": user["email"], "created_at": created})
        if purchase_docs: await db.purchases.insert_many(purchase_docs)
        expense_docs = [{"id": new_id(), "business_id": bid, "category": cat, "description": desc, "amount": amount, "date": (now - timedelta(days=days_ago)).date().isoformat(), "user_email": user["email"], "created_at": ts(days_ago, 18)} for cat, desc, amount, days_ago in profile["expenses"]]
        if expense_docs: await db.expenses.insert_many(expense_docs)
        stock_map[products[10]["id"]] = 0; stock_map[products[3]["id"]] = 2; stock_map[products[4]["id"]] = 4
        for p in products: await db.products.update_one({"id": p["id"]}, {"$set": {"stock": stock_map[p["id"]], "updated_at": now_iso()}})
...
async def seed_all():
    ...
    demo_profiles = [("kiosco.demo@controlpyme.com", "Luis Martínez", KIOSCO, 21), ("verduleria.demo@controlpyme.com", "Rosa Jiménez", VERDULERIA, 33), ("repuestos.demo@controlpyme.com", "Carlos Pirela", REPUESTOS, 47)]
...
async def reset_demo_state():
    ...
    expected_demo_names = {FERRETERIA["name"], KIOSCO["name"], VERDULERIA["name"], REPUESTOS["name"]}
    stale_businesses = await db.businesses.find({"name": {"$nin": list(expected_demo_names)}}, {"id": 1, "owner_id": 1, "name": 1}).to_list(length=None)
    stale_business_ids = [b["id"] for b in stale_businesses if b.get("id")]
    if stale_business_ids:
        await db.products.delete_many({"business_id": {"$in": stale_business_ids}}); await db.sales.delete_many({"business_id": {"$in": stale_business_ids}}); await db.purchases.delete_many({"business_id": {"$in": stale_business_ids}}); await db.expenses.delete_many({"business_id": {"$in": stale_business_ids}}); await db.inventory_movements.delete_many({"business_id": {"$in": stale_business_ids}}); await db.businesses.delete_many({"id": {"$in": stale_business_ids}})
    demo_emails = [os.environ.get("ADMIN_EMAIL"), "kiosco.demo@controlpyme.com", "verduleria.demo@controlpyme.com", "repuestos.demo@controlpyme.com"]; demo_emails = [email for email in demo_emails if email]
    for collection_name in ["sales", "purchases", "expenses", "products", "inventory_movements", "assistant_messages", "login_attempts"]: await getattr(db, collection_name).delete_many({})
    user_ids = [u["id"] async for u in db.users.find({"email": {"$in": demo_emails}}, {"id": 1})]
    if user_ids: await db.businesses.delete_many({"owner_id": {"$in": user_ids}})
    ...
    if admin_email and admin_password:
        admin = await _ensure_user(admin_email, "David Arenas", admin_password, role="propietario", platform_role="superadmin"); await _ensure_demo_business(admin, FERRETERIA, 7, force_rebuild=True)
    for email, name, profile, rng_seed in [("kiosco.demo@controlpyme.com", "Luis Martínez", KIOSCO, 21), ("verduleria.demo@controlpyme.com", "Rosa Jiménez", VERDULERIA, 33), ("repuestos.demo@controlpyme.com", "Carlos Pirela", REPUESTOS, 47)]:
        user = await _ensure_user(email, name, DEMO_PASSWORD); await _ensure_demo_business(user, profile, rng_seed, force_rebuild=True)
    await db.businesses.update_many({"active": {"$exists": False}}, {"$set": {"active": True}}); await db.users.create_index("email", unique=True)
    remaining_names = [b["name"] async for b in db.businesses.find({"name": {"$in": list(expected_demo_names)}}, {"name": 1})]
    if sorted(set(remaining_names)) != sorted(list(expected_demo_names)): raise RuntimeError(f"No quedó el estado demo esperado: {sorted(set(remaining_names))}")
