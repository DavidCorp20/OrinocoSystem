...
def _sheet(wb, title, headers, rows):
    ...
    if rows:
        ...
        table = Table(displayName=(title.replace(" ", "") + "Reporte")[:25], ref=ref); table.tableStyleInfo = TableStyleInfo(name="TableStyleMedium2", showRowStripes=True); ws.add_table(table)
    for i, h in enumerate(headers, 1):
        vals = [len(str(h))] + [len(str(r[i-1])) for r in rows[:100] if i-1 < len(r)]
        ws.column_dimensions[get_column_letter(i)].width = max(12, min(36, max(vals) + 3))
...
async def export_finances_xlsx(from_date: str | None = None, to_date: str | None = None, user: dict = MANAGER):
    ...
    sales = await db.sales.find(q, {"_id": 0}).sort("created_at", -1).to_list(50000)
    expenses = await db.expenses.find(q, {"_id": 0}).sort("created_at", -1).to_list(50000)
    purchases = await db.purchases.find(q, {"_id": 0}).sort("created_at", -1).to_list(50000)
    revenue = round(sum(float(s.get("total", 0) or 0) for s in sales), 2)
    gross = round(sum(float(s.get("profit", 0) or 0) for s in sales), 2)
    opex = round(sum(float(e.get("amount", 0) or 0) for e in expenses), 2)
    purchases_total = round(sum(float(p.get("total", 0) or 0) for p in purchases), 2)
    ...
    summary["B7"].number_format = "0.0%"
    for ws in wb.worksheets:
        if ws.title == "Gastos por categoría":
