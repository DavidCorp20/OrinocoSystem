...
async def create_product(data: ProductIn, user: dict = MANAGER):
    bid=user["business_id"];count=await db.products.count_documents({"business_id":bid})
...
async def delete_product(product_id:str,user:dict=MANAGER):
    ...
    if result.deleted_count==0: raise HTTPException(status_code=404,detail="Producto no encontrado")
    return {"ok":True}
