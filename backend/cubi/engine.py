...
async def build_business_insights(db, business_id: str) -> dict:
    ...
    # Keep the two most useful rankings available to Cubi: sales volume and revenue.
    top_by_units = sorted(abc, key=lambda x: x["units"], reverse=True)[:10]
    top_by_revenue = sorted(abc, key=lambda x: x["revenue"], reverse=True)[:10]
    ...
    return {
        ...
        "summary": {"revenue": total_revenue, "profit": total_profit, "products_sold": len(product_metrics)},
        "top_products": top_by_units,
